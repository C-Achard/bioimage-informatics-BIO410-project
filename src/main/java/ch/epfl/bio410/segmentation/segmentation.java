...
public class segmentation {
    ...
    public static void segment(ImagePlus imp) {
        ...
        // find the closest bacteria on next frame by computing euclidian distance between centroid of current and all bacteria on next frame
        List<List<String>> new_bacteria_labels = results;

        // loop over all frames
        for (int frame = 0; frame < summary.size(); frame++) {
            ...
            ???get_bacteria_ids_of_frame(frame, results);
            for (int i = 0; i <= bact_number_current_frame; i++) {
                int closest_bacteria_label = label_bacteria(i, frame, frame + 1, results);
                // do something with closest_bacteria_label
            }
        }
    }
     ...
     */
    public static int label_bacteria(int bacteria_label, int frame1, int frame2, List<List<String>> results){
        double[] current_centroid = get_centroid(bacteria_label, results);
        double min_distance = Double.MAX_VALUE;
        int closest_bacteria_label = -1;

        for (int i = 0; i < get_bact_count(results, frame2); i++) {
            double[] next_centroid = get_centroid(i, results);
            double distance = euclidian_distance(current_centroid, next_centroid);

            if (distance < min_distance) {
                min_distance = distance;
                closest_bacteria_label = i;
            }
        }
        return closest_bacteria_label;
